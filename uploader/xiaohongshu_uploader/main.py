...
class XiaoHongShuVideo(XiaoHongShuBaseUploader):
    ...
    async def upload_video_content(self, page: Page) -> None:
        ...
        if self.dry_run:
            screenshot_path = f"/tmp/xiaohongshu_dry_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            await page.screenshot(path=screenshot_path, full_page=True)
            xiaohongshu_logger.info(_msg("🧪", "Dry run 模式：跳过发布，所有基础设置已完成"))
            xiaohongshu_logger.info(_msg("📸", f"截图已保存: {screenshot_path}"))
            return
    ...
    async def upload_in_session(self, session) -> dict:
        ...
        try:
            ...
            if page and not page.is_closed():
                xiaohongshu_logger.info(_msg("📌", "小红书视频发布页面已保留供人工复核"))
...
class XiaoHongShuNote(XiaoHongShuBaseUploader):
    ...
    async def upload_note_content(self, page: Page) -> None:
        ...
        if self.dry_run:
            screenshot_path = f"/tmp/xiaohongshu_note_dry_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            await page.screenshot(path=screenshot_path, full_page=True)
            xiaohongshu_logger.info(_msg("🧪", "Dry run 模式：跳过发布，所有基础设置已完成"))
            xiaohongshu_logger.info(_msg("📸", f"截图已保存: {screenshot_path}"))
            return
